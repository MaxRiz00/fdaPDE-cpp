...
#ifndef __iso_ls_elliptic_SOLVER_H__
#define __iso_ls_elliptic_SOLVER_H__
...
struct iso_ls_elliptic {
   private:
    ...
    void eval_basis_at_(const DataLocs& locs) {
        //std::cout << "Evaluating basis at " << locs.rows() << " locations." << std::endl;
        fdapde_assert(n_locs_ == locs.rows());
        if constexpr (std::is_same_v<DataLocs, matrix_t>) {   // pointwise sampling
            //std::cout << "Evaluating pointwise basis at " << locs.rows() << " locations." << std::endl;
        ...
        // print psi
        //std::cout << "Psi_ size: " << Psi_.rows() << " x " << Psi_.cols() << std::endl;
        //std::cout << "Psi_:\n" << Psi_ << std::endl;
        return;
    }
    ...
    template <typename GeoFrame> void eval_basis_at_(const GeoFrame& gf) {
        ...
        case ltype::point: {
            ...
            //if (spatial_index.points_at_dofs()) {
                //std::cout<< "Evaluating basis at qiqiqiqi" <<  std::endl;
            //    Psi_.resize(n_locs_, n_dofs_);
            //    Psi_.setIdentity();
            //} else {
            //std::cout << "Evaluating basis at " << spatial_index.rows() << " locations." << std::endl;
                Psi_ = point_eval_(spatial_index.coordinates());
                //std::cout << "Psi_ size: " << Psi_.rows() << " x " << Psi_.cols() << std::endl;
                //std::cout<<Eigen::MatrixXd(Psi_)<<std::endl;
            //std::cout << "Evaluated basis at " << Psi_.rows() << " locations." << std::endl;
            //}
            //std::cout << "Psi_ size: " << Psi_.rows() << " x " << Psi_.cols() << std::endl;
            // std::cout << "Psi_:\n" << Eigen::MatrixXd(Psi_) << std::endl;
            D_ = vector_t::Ones(n_locs_).asDiagonal();
            break;
        }
        /*
   ...
   public:
    ...
    iso_ls_elliptic() noexcept = default;
    ...
    iso_ls_elliptic(const std::string& formula, const GeoFrame& gf, InfoT&& info, const WeightMatrix& W) : W_(W) {
	...
	n_locs_ = n_obs_;
        ...
        //std::cout << "Evaluating basis at geoframe..." << std::endl;
	analyze_data(formula, gf, W);
    }
    template <typename GeoFrame, typename InfoT>
        requires(is_valid_info_t<InfoT>::value)
    iso_ls_elliptic(const std::string& formula, const GeoFrame& gf, InfoT&& info) :
        iso_ls_elliptic(formula, gf, info, vector_t::Ones(gf[0].rows()).asDiagonal()) { }
    // construct with no data
    template <typename GeoFrame, typename InfoT, typename WeightMatrix>
        requires(is_valid_info_t<InfoT>::value)
    iso_ls_elliptic(const GeoFrame& gf, InfoT&& info, const WeightMatrix& W) : W_(W) {
        fdapde_static_assert(GeoFrame::Order == 1, THIS_CLASS_IS_FOR_ORDER_ONE_GEOFRAMES_ONLY);
        fdapde_assert(gf.n_layers() == 1);
        n_obs_  = gf[0].rows();
	n_locs_ = n_obs_;
    ...
    template <typename GeoFrame, typename InfoT>
        requires(is_valid_info_t<InfoT>::value)
    iso_ls_elliptic(const GeoFrame& gf, InfoT&& info) :
        iso_ls_elliptic(gf, info, vector_t::Ones(gf[0].rows()).asDiagonal()) { }
    ...
    template <typename Penalty> void discretize(Penalty&& penalty) {
        ...
        //std::cout << "Discretizing iso_ls_elliptic solver with penalty pair: " << std::endl;
        using BilinearForm = std::tuple_element_t<0, std::decay_t<Penalty>>;
        using LinearForm = std::tuple_element_t<1, std::decay_t<Penalty>>;
        using IsoSpace = typename BilinearForm::TrialSpace;
        using TrialFunction = TrialFunction<IsoSpace, iso_tag>;
        using TestFunction = TestFunction<IsoSpace, iso_tag>;
	// discretization

        const BilinearForm& bilinear_form = std::get<0>(penalty);
        const LinearForm& linear_form = std::get<1>(penalty);
        auto Vh = bilinear_form.trial_space();
        auto& dof_handler = Vh.dof_handler();
        n_dofs_ = dof_handler.n_mapped_dofs();   // number of basis functions over physical domain
        //std::cout<< "Discretizing iso_ls_elliptic solver with " << n_dofs_ << " dofs." << std::endl;
        //internals::fe_mass_assembly_loop<Space> mass_assembler(bilinear_form.trial_space());

        TrialFunction u(Vh);
        TestFunction v(Vh);
        //std::cout << "Discretizing bilinear form..." << std::endl;
        //auto mesh = bilinear_form.mesh();
        //auto mass_assembler = integral(bilinear_form.mesh()) (u * v );
        //std::cout << "Assembling mass matrix..." << std::endl;
        // put zero
        //R0_ = sparse_matrix_t::Zero(n_dofs_, n_dofs_);
        //std::cout << "Assembling stiffness matrix..." << std::endl;
        R1_ = bilinear_form.assemble();
        //load R1_ from file
        //
       //Eigen::SparseMatrix<double> R1;
    //Eigen::loadMarket(R1_, "../R1_4.mtx");
       //Eigen::saveMarket(R1_, "../R1_4.mtx");
        ...
        //dof_handler.enforce_periodic_constraints(R0_);

        //dof_handler.enforce_periodic_constraints(u_);
        //std::cout<< "R0_ (mass matrix): " << R0_.rows() << " x " << R0_.cols() << std::endl;
        //std::cout<< "R1_ (stiffness matrix): " << R1_.rows() << " x " << R1_.cols() << std::endl;

        dof_handler.enforce_periodic_constraints(R1_);
        //std::cout<< "R1_ (stiffness matrix): " << R1_.rows() << " x " << R1_.cols() << std::endl;

        // print first 2 rows of R0_
        //std::cout << "R0_ first 2 rows: " << std::endl;
        for (int i = 0; i < 2; ++i) {
            //std::cout << R1_.row(i) << std::endl;
        }

	// store handles for basis system evaluation at locations
        point_eval_ = [iso_space = bilinear_form.trial_space()](const matrix_t& locs) -> decltype(auto) {
            return internals::point_basis_eval(iso_space, locs);
        };
        //std::cout << "Basis system discretization complete." << std::endl;
        /*
        areal_eval_ = [iso_space = bilinear_form.trial_space()](const binary_t& locs) -> decltype(auto) {
            return internals::areal_basis_eval(iso_space, locs);
        };
        */
	b_.resize(n_dofs_, 1);
    ...
    //std::cout<< "Discretization complete with dofs." << std::endl;
        return;
    }
    // non-parametric fit
    ...
    void analyze_data(const DataLocs& locs, const matrix_t& y, const WeightMatrix& W) {
        //std::cout << "Analyzing data with " << n_obs_ << " observations." << std::endl;
        fdapde_assert(
    ...
    void analyze_data(const DataLocs& locs, const matrix_t& y, const matrix_t& X, const WeightMatrix& W) {
        fdapde_assert(
          ...
          //std::cout << "Analyzing data with formula: " ;
        n_obs_  = locs.rows();
	n_locs_ = n_obs_;
    ...
    void analyze_data(const std::string& formula, const GeoFrame& gf, const WeightMatrix& W) {
        //std::cout << "Analyzing data with formula1: ";
        fdapde_static_assert(GeoFrame::Order == 1, THIS_CLASS_IS_FOR_ORDER_ONE_GEOFRAMES_ONLY);
        fdapde_assert(gf.n_layers() == 1);
        n_obs_  = gf[0].rows();
	n_locs_ = n_obs_;
    //std::cout<< " with " << n_obs_ << " observations." << std::endl;
	...
	W_ /= n_obs_;
        if (n_covs_ == 0) {
            ...
            /// print first 2 rows of invXtWXXtW_
    ...
    std::pair<vector_t, vector_t> fit(double lambda) {
        ...
        if (lambda_saved_.value() != lambda || W_changed_) {
            ...
            // size of PSI_NA
        //std::cout << "Size of PsiNA: " << PsiNA().rows() << " x " << PsiNA().cols() << std::endl;
            ...
            sparse_matrix_t A = PsiNA().transpose() * D_ * W_ * PsiNA() + lambda * R1_;
              ...
              //
              //std::cout<<"first rows of A: " << R1_.topRows(2) << std::endl;
	    enforce_lhs_dirichlet_bc_(A);
            invA_.compute(A);
            //std::cout << "System matrix assembled and factorized." << std::endl;
	    W_changed_ = false;
        }
        if (lambda_saved_.value() != lambda) {
            ...
            for (int i = 0; i < dirichlet_dofs_.size(); ++i) { 
                //std::cout << "Enforcing Dirichlet BC at dof: " << dirichlet_dofs_[i] << std::endl;
                b_.row(dirichlet_dofs_[i]).setZero(); }
        }
        lambda_saved_ = lambda;
        vector_t x;
        if (n_covs_ == 0) {
            //std::cout << "Solving nonparametric system..." << std::endl;
            //std::cout <<"B_ : "<<b_<<std::endl;
            x = invA_.solve(b_);
            f_ = x.topRows(n_dofs_);
        } else {
            //std::cout << "dims of XtWX_: " << XtWX_.rows() << " x " << XtWX_.cols() << std::endl;
            // head of XtWX_ is the design matrix
            //std::cout<< "Ecco: "<< XtWX_.topRows(2) << std::endl;
            x = woodbury_system_solve(invA_, U_, XtWX_, V_, b_);
            f_ = x.topRows(n_dofs_);
            beta_ = invXtWXXtW_ * (y_ - Psi_ * f_);
        }
        g_ = x.bottomRows(n_dofs_);
        //std::cout<<"ECCO BENE"<<std::endl;
    ...
    double edf(int r = 100, int seed = random_seed) {
        ...
        if (n_covs_ == 0) {
            Bs_->topRows(n_dofs_) = PsiNA().transpose() * D_ * W_ * (*Us_); /// il meno c'è ???????????? 
        } else {
	...
	// enforce Dirichlet BCs, if any
        ...
        //std::cout << "Tr[S] approximation: " << trS/r << std::endl;
        // Compute and print exact trace of S for debugging (only for n_covs_ == 0)
        if (n_covs_ == 0) {
            //matrix_t S = Psi_ * invA_.solve(Psi_.transpose() * D_ * W_);
            //std::cout << "Tr[S] (exact): " << S.diagonal().sum() << std::endl;
        }
    ...
    double edf(const LambdaT& lambda, int r = 100, int seed = random_seed) {
        ...
        if (lambda_saved_.value() != lambda_) {
            //std::cout << "Recomputing system matrix for edf with lambda: " << lambda_ << std::endl;
            //SparseBlockMatrix<double, 2, 2> A(
...
template <typename BilinearForm, typename LinearForm> struct iso_ls_elliptic {
    using solver_t = internals::iso_ls_elliptic;
   private:
    struct info_t {
        std::tuple<BilinearForm, LinearForm> penalty;
    };
   public:
    iso_ls_elliptic(const BilinearForm& bilinear_form, const LinearForm& linear_form) :
...
#endif // __iso_ls_elliptic_SOLVER_H__
